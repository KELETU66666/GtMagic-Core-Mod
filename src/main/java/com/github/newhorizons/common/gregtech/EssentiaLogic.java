...
public class EssentiaLogic {
    ...
    public long thermalEssentia(Aspect aspect) {
        ...
        if (depleteInput(GTMMaterial.SuperCoolant.getFluid(ceoInput))) {
            ceoOutput = 9.0D;
        } else if (depleteInput(GTMMaterial.Cryotheum.getFluid(ceoInput))) {
            ceoOutput = 5.0D;
        } else if (depleteInput(GTMMaterial.Coolant.getFluid(ceoInput))) {
            ceoOutput = 1.5D;
    ...
    public long victusEssentia(Aspect aspect) {
        ...
        if (depleteInput(GTMMaterial.XPJuice.getFluid(ceoInput))) {
            ceoOutput = 2.0D;
        } else if (depleteInput(GTMMaterial.Blood.getFluid(ceoInput))) {
            ceoOutput = 6.0D;
        }
        return (long) (baseValue * ceoOutput);
    }
    ...
    public long spiritEssentia(Aspect aspect) {
        ...
        double ceoOutput = 1.0D;
        int ceoInput = (int) LargeEssentiaEnergyData.getAspectCeo(aspect) * 2;
        if (depleteInput(GTMMaterial.Spirit.getFluid(ceoInput))) {
            ceoOutput = 10D * (1 + mStableValue / 100D);
        } else if (depleteInput(GTMMaterial.Hollowtears.getFluid(ceoInput))) {
    ...
    public long radiationEssentia(Aspect aspect) {
        ...
        if (depleteInput(Materials.Caesium.getFluid(ceoInput))) {
            ...
            ceoOutput = 4.0D;
        } else if (depleteInput(GTMMaterial.AtomicSeparationCatalyst.getFluid(ceoInput))) {
            ceoOutput = 16.0D;
        }
        return (long) (baseValue * ceoOutput);
    }
